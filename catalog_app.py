...
"""
    ...
    if result['user_id'] != gplus_id:
        response = make_response(
            'Token user ID doesnt match given user ID', 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
"""
    ...
    if access_token is None:
        response = make_response(json.dumps(
            'User is already disconnected'), 400)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def add_item():
    """
    Return the page the add an item or the method to save the item in
    the database depending
    on the http method.
    """
    categories = session.query(Category).all()
    ...
    if request.method == 'POST':
        ...
        session.add(Item(title=title, description=description,
        ...
        return render_template(
                                'add-item.html', categories=categories,
                                login_session=login_session)
...
def remove_item(item_title):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('get_catalog_items',
                                category_title=category_title))
